...
def parse_certificate(file_path):
    ...
    # Удаляем все, что в скобках, из текста перед поиском стандартов
...
def test_parse(file_path):
    """Тестовая функция для одного файла"""
    print(f"\n{'=' * 60}")
    print(f"Файл: {file_path}")
    print('=' * 60)

    try:
        result = parse_certificate(file_path)

        print(f"\n📊 КОДЫ ТНВЭД ({len(result['tnved_codes'])}):")
        for code in result['tnved_codes']:
            print(f"  - {code}")

        print(f"\n📋 СТАНДАРТЫ ({len(result['standards'])}):")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        pattern = "data/*Макет СС ТР ЕАЭС*.docx"
        files = glob.glob(pattern)

        if not files:
            print("❌ Файлы не найдены. Проверьте папку data/")
        else:
